...
def test_install_manifest_explicit_dir(tmp_path):
    ...
    target = adbc_driver_db2.install_manifest(directory=tmp_path)
    assert target == tmp_path / "db2.toml"
    text = target.read_text(encoding="utf-8")
    assert "manifest_version = 1" in text
    assert "entrypoint = 'Db2DriverInit'" in text
    ...
    lib_line = next(
        line for line in text.splitlines() if "libadbc_driver_db2" in line
    )
...
def test_install_manifest_cli(tmp_path, capsys):
    from adbc_driver_db2.__main__ import main

    rc = main(["install-manifest", "--dir", str(tmp_path)])
    assert rc == 0
    assert (tmp_path / "db2.toml").is_file()
    assert str(tmp_path / "db2.toml") in capsys.readouterr().out


def test_manifest_dir_scopes(monkeypatch, tmp_path):
    from adbc_driver_db2 import _manifest
