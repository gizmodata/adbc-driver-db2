"""Command-line helpers: ``python -m adbc_driver_db2 install-manifest``."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m adbc_driver_db2")
    sub = parser.add_subparsers(dest="command", required=True)

    install = sub.add_parser(
        "install-manifest",
        help=(
            "Write the db2.toml ADBC driver manifest so connection "
            "profiles and db2:// URIs resolve this driver by name."
        ),
    )
