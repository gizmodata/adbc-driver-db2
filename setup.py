"""
setup.py for the adbc-driver-db2 Python wheel.
...
via the ``ADBC_DB2_LIBRARY`` environment variable, mirroring the
...
package_dir = source_root / "python" / "adbc_driver_db2"
...
# Bundle the c-shared library produced by `make -C pkg/db2`.
library = os.environ.get("ADBC_DB2_LIBRARY")
target = package_dir / f"libadbc_driver_db2.{_library_suffix()}"
...
if library:
    ...
    if not library_path.is_file():
        raise ValueError(
            f"ADBC_DB2_LIBRARY points at {library_path!r} but no file is there. "
            f"Original env value: {os.environ.get('ADBC_DB2_LIBRARY')!r}"
        )
    ...
    pass  # building sdist — skip the ADBC_DB2_LIBRARY check
elif not target.is_file():
    raise ValueError(
        "ADBC_DB2_LIBRARY env var is required when building a wheel "
        "(it should point to the libadbc_driver_db2.{so,dylib,dll} "
        "produced by `make -C pkg/db2`)."
    )
